...
public class DepartamentoTest implements TestesModel {
    ...
    public void teste_annotation_validator_incorreto() { // Código Retirado do Stack Overflow
        Departamento exDepErrado = Fixture.from(Departamento.class).gimme("invalido");
    ...
    public void deve_criar_departamento_valido_atraves_deConstrutor_utilizando_Fixture() {
        dep = new Departamento(dep.getIdDepartamento(), dep.getNome(), dep.getRamal());
    ...
    public void nao_deve_registrar_departamento_com_RAMAL_jaUtilizado() {
        dep.registraDep(dep.getIdDepartamento(), dep.getNome(), dep.getRamal())
        .registraDep(1 + dep.getIdDepartamento(), "foo", dep.getRamal());
    }

    @Test(expected = NullPointerException.class)
    public void nao_deve_criar_departamento_nulo() {
        dep = new Departamento(NULLONG, NULLSTR, NULLINTEGER);
    ...
    @Test (expected = IllegalArgumentException.class)
    public void deve_gerar_exception_casoDepartamento_solicitado_naoFor_existente() {
        Departamento.solicitaDep(1050);
    }
    ...
    public void deve_gerarConstrain_seRegistrar_nome_comNumeros() {
        Set<ConstraintViolation<Departamento>> constraintViolations = validator.validate(dep.registraDep(dep.getIdDepartamento(), "Cleber4578Araujo", dep.getRamal()));        
        assertThat(constraintViolations, is(not(equalTo(null)))); exibeConstrains(constraintViolations);
